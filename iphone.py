...
def consolidate_similar_colors(df, price_tolerance=0.02):
    """
    Consolidate products with same specs but different colors when prices are similar
    This reduces visual noise in the output while preserving important price differences
    
    Parameters:
    df (pd.DataFrame): DataFrame with individual color variants  
    price_tolerance (float): Max price difference ratio to consolidate (default 2%)
    ...
    # Color patterns for extraction and removal
    color_patterns = [
        # Multi-word colors (match first to avoid partial matches)
        r'\b(cosmic\s+orange|deep\s+blue|light\s+gold|cloud\s+white|space\s+black|sky\s+blue)\b',
        r'\b(space\s+gray|rose\s+gold|alpine\s+green|sierra\s+blue|product\s+red|mist\s+blue)\b',
        r'\b(natural\s+titanium|black\s+titanium|white\s+titanium|blue\s+titanium|desert\s+titanium)\b',
        # Single word colors
        r'\b(black|white|blue|green|pink|yellow|red|purple|orange|gray|grey|silver|gold)\b',
        r'\b(midnight|starlight|natural|desert|graphite|ultramarine|lavender|sage|teal)\b'
    ]
    ...
    for idx, row in df_copy.iterrows():
        base_name, colors = extract_base_name_and_colors(row['PRODUCT_NAME'])
        df_copy.at[idx, 'Base_Name'] = base_name
        df_copy.at[idx, 'Colors'] = colors
    ...
    for base_name, group in df_copy.groupby('Base_Name'):
        if len(group) <= 1:
            ...
            # Multiple variants - check if prices are similar enough to consolidate
            price_cols = [col for col in group.columns if col.startswith('Price_') and col.endswith(('_US', '_TW'))]
            ...
            if should_consolidate and len(group) > 1:
                ...
                # Collect all colors
                all_colors = []
                for colors_list in group['Colors']:
                    if isinstance(colors_list, list):
                        all_colors.extend(colors_list)
                
                unique_colors = list(set(all_colors))
                
                # Simply use the base name without any color information
                consolidated['PRODUCT_NAME'] = base_name
