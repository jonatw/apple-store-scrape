#!/usr/bin/env python3
"""
Optimized Mac scraper with improved specification extraction
This version uses better matching between JSON products and HTML specifications
"""
    ...
    try:
        ...
        for link in soup.find_all('a', href=True):
            ...
            if '/shop/buy-mac/' in href:
                ...
                if not any(x in model for x in ['compare', 'accessories', 'help', 'financing', 'display']):
                    mac_links.append(model)

        unique_models = list(set(mac_links))
        if unique_models:
            # Filter to main Mac computers only
            main_models = [m for m in unique_models if m in ['mac-mini', 'imac', 'mac-studio', 'macbook-air', 'macbook-pro']]
...
def extract_product_details(url, region_code=""):
    """Extract product details with enhanced specification matching"""
...
def main():
    ...
    print("Starting Optimized Mac Product Scraper...")
    ...
    # Save to CSV
    output_file = 'optimized_mac_products.csv'
